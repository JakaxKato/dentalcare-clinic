...
const Tooth = ({ fdi, tooth, onClick, readOnly }) => {
  ...
  return (
    ...
    >
      ...
      <span className="text-[10px] text-stone-500 mt-0.5">{fdi}</span>
    </button>
  );
};
...
const Odontogram = ({ value = [], onChange, readOnly = false }) => {
  ...
  return (
    <div className="bg-white rounded-xl p-4 border border-stone-200">
      <div className="text-center text-xs text-stone-500 mb-1">Atas (Maxilla)</div>
      ...
      <div className="text-center text-xs text-stone-500 mt-1">Bawah (Mandibula)</div>

      <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-stone-100">
        {CONDITION_OPTIONS.filter((o) => o.value !== 'healthy').map((opt) => (
          <span key={opt.value} className="inline-flex items-center gap-1.5 text-xs text-stone-600">
        ...
        {current && (
          <div className="space-y-4">
            ...
            <label className="block">
              <span className="block text-sm font-medium text-stone-700 mb-1">Catatan</span>
              <textarea
                rows={2}
                value={current.notes || ''}
